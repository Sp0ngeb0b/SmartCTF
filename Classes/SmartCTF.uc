...
var()                config float insaneComboPlayerMovementLimit;
var()                config float insaneComboBallMovementLimit;
var()                config float insaneComboZLimit;
var()                config float insaneComboXYLimit;
   ...
   OtherStats = SCTFGame.GetStats( Other );
      ...
      if(OtherStats!=none && bStoreStats && !Level.Game.bGameEnded && Other!=none && Other.bIsPlayer&& !Other.IsA('Spectator') && !Other.IsA('Bot')	&& Other.PlayerReplicationInfo!=none&& Other.PlayerReplicationInfo.PlayerName!="Player" )
	{
		...
		for(i=0; i<32 && GoneName[i]!=""; i++)
            ...
            OtherStats.Combos=GoneStats[i].Combos;
            OtherStats.InsaneCombos=GoneStats[i].InsaneCombos;
...
function bool PreventDeath( Pawn Victim, Pawn Killer, name DamageType, vector HitLocation )
{
  ...
  if( !bWarmupSkip )
  {
    ...
    if( VictimPRI.HasFlag != None )
    {
      ...
      // Denied?
      if(!bTooCloseForSaves && CTFReplicationInfo( Level.Game.GameReplicationInfo ).FlagList[VictimPRI.Team].bHome && VSize(Victim.Location - FlagStands[VictimPRI.Team].Location ) < 200 ) {
        KillerPRI.Score += 2;
        
        if( Level.Game.LocalLog != None ) Level.Game.LocalLog.LogSpecialEvent( "flag_kill_denied", KillerPRI.PlayerID, VictimPRI.PlayerID, VictimPRI.Team );

        if( SavedMsgType == 1 && PlayerPawn( KillerPRI.Owner ) != None ) PlayerPawn( KillerPRI.Owner ).ClientMessage( class'SmartCTFMessage'.static.GetString( 8 + 64, KillerPRI ) );
        else if( SavedMsgType == 2 ) BroadcastMessage( class'SmartCTFMessage'.static.GetString( 8, KillerPRI ) );
        else if( SavedMsgType == 3 ) BroadcastLocalizedMessage( class'SmartCTFMessage', 8, KillerPRI );
        if( bPlaySavedSound && PlayerPawn( KillerPRI.Owner ) != None ) PlayerPawn( KillerPRI.Owner ).ReceiveLocalizedMessage( class'SmartCTFAudioMsg', 8 );
      }
  ...
  if( DamageType == 'combo') {
    ...
    foreach VisibleCollidingActors(class 'ShockProj', proj, 200+50, HitLocation, false) {
      if(proj.Instigator != None && proj.Instigator == Killer && vector(proj.getItemName("")) != tmpVector) {
        tmpVector = vector(proj.getItemName("")); // This is the initial spawn location of the projectile
        XYVector.X = Killer.Velocity.X;
        XYVector.Y = Killer.Velocity.Y;
                
        // Do we have an insane combo?
        if(VSize(Killer.Location - tmpVector) >= insaneComboPlayerMovementLimit || VSize(proj.Location - tmpVector) > insaneComboBallMovementLimit ||
           Abs(Killer.Velocity.Z) >= insaneComboZLimit || VSize(XYVector) >= insaneComboXYLimit || VSize(Killer.Velocity) >= insaneComboVelLimit ) {
          KillerPRI.Score += 1;
          KillerStats.InsaneCombos++;
...
defaultproperties
{
     ...
     insaneComboPlayerMovementLimit=200
     insaneComboBallMovementLimit=2500
     insaneComboZLimit=300
     insaneComboXYLimit=800
